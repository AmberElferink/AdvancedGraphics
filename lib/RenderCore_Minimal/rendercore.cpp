...
void RenderCore::SetTextures( const CoreTexDesc *tex, const int textures )
{
	...
	{
		Texture *t;
		if ( i < raytracer.scene.texList.size() )
			t = raytracer.scene.texList[i];
		else
			raytracer.scene.texList.push_back( t = new Texture() );
		t->pixels = (uint *)MALLOC64( tex[i].pixelCount * sizeof( uint ) );
		if ( tex[i].idata )
			memcpy( t->pixels, tex[i].idata, tex[i].pixelCount * sizeof( uint ) );
		else
			memcpy( t->pixels, 0, tex[i].pixelCount * sizeof( uint ) /* assume integer textures */ );
...
float RenderCore::ComputeArea( float3 vertex0, float3 vertex1, float3 vertex2 )
{
